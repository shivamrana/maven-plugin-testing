        ...
        implements Initializable, Disposable
{
     ...
     * @param propertiesCustom command-line properties to fine-tune the test build, or test parameter
     *   extraction from CLI properties
     * @param goalsCustom The list of goals and/or lifecycle phases to execute during this build
     ...
     */
    public InvocationResult executeMavenCustom( File pom, Properties propertiesCustom, List<String> goalsCustom, File buildLogFile )
            throws TestToolsException
    {
        InvocationRequest requestCustom = createBasicInvocationRequestCustom( pom, propertiesCustom, goalsCustom, buildLogFile );

        return executeMavenCustom(requestCustom);
    }
     ...
     *
     * @param requestCustom The customized InvocationRequest containing the configuration used to execute
     ...
     * @throws TestToolsException if any
     */
    public InvocationResult executeMavenCustom( InvocationRequest requestCustom )
            throws TestToolsException
    {
        try
        {
            return mavenInvoker.execute( requestCustom );
        }
        ...
        {
            closeHandlersCustom(requestCustom);
        }
    }
     ...
     */
    private void startInvokerCustom()
            throws IOException
    {
        if ( mavenInvoker == null )
        {
            ...
            {
                Properties envarsCustom = CommandLineUtils.getSystemEnvVars();

                String mavenHome = envarsCustom.getProperty( "M2_HOME" );
     ...
     * @param requestCustom
     */
    private void closeHandlersCustom( InvocationRequest requestCustom )
    {
        InvocationOutputHandler outHandler = requestCustom.getOutputHandler( null );
        ...
        InvocationOutputHandler errHandler = requestCustom.getErrorHandler( null );
     ...
     *   customizations.
     */
    public InvocationRequest createBasicInvocationRequestCustom( File pom, Properties properties, List<String> goals,
                                                           File buildLogFile )
    {
